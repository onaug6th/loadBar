...
(function init(){
    ...
    logger("copy路径从", resolve('node_modules/jquery/dist/'), "到", resolve('dist/assets/jquery/js/'));

    logger("资源文件引用路径为", resolve('assets/jquery/js/jquery.js'));
...
if (dev) {
    plugins = [
        ...
        new CopyWebpackPlugin([
            { 
                ...
                to: resolve('dist/assets/jquery/js/'),
                toType: "dir"
            }
        ]),
        ...
        /**
         * 打包时自动引入资源文件
         * 注意！！ 这个plugins在webpack4之后需要在HtmlWebpackPlugin之后使用
         */
        new HtmlWebpackIncludeAssetsPlugin({
            assets: [
                'assets/jquery/js/jquery.js'
            ],
            //  设置为false，会将资源文件插入于入口文件前
            append: false
        })
    ]
} else {
    plugins = [
        new CopyWebpackPlugin([
            { from: 'node_modules/jquery/dist/', to: 'example/assets/plugins/jquery/js/' }
        ]),
        new HtmlWebpackIncludeAssetsPlugin({
            assets: [
                'example/assets/plugins/jquery/js/jquery.min.js'
            ],
            append: false //资源置于构建类库之前
        }),
        new HtmlWebpackPlugin({
            filename: 'index.html',
            template: './src/index.html'
...
if (dev) {
    ...
    const convertObj = convert(historyObj)
